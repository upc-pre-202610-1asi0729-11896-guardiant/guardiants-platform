...
import com.guardiants.platform.shared.interfaces.rest.resources.ErrorResource;
...
public final class ErrorResponseAssembler {
     ...
     */
    public static ResponseEntity<ErrorResource> toErrorResponseFromApplicationError(
    ...
    private static String toLocalizedMessageFromApplicationError(ApplicationError error) {
        ...
        String specificMessage = toLocalizedMessageOrNull(specificKey,
        ...
        return toLocalizedMessageWithFallback(fallbackKey, error.message(),
                error.details(), toEntityNameFromErrorCode(error.code()));
    }

    private static String toSpecificMessageKeyFromErrorCode(String errorCode) {
        return "error.%s.message".formatted(
    ...
    private static String toLocalizedMessageWithFallback(String key, String fallback,
                                                         Object... args) {
        Locale locale = LocaleContextHolder.getLocale();
        try {
            ResourceBundle bundle = ResourceBundle.getBundle(MESSAGES_BASENAME, locale);
